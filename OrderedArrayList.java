...
public class OrderedArrayList {
	...
	for (int ctr = 0; ctr < _data.size(); ctr++){
	    if (_data[ctr].compareTo(target) == 0) {return ctr;}
	}
	return -1;
	...
	int med = 0;
	int hi = _data.size()-1;
	return findBinH( target, lo, med, hi);
    }	
    
    public int findBinH( Comparable target, int lo, int med, int hi){
	if (lo < hi) {
	    int med = (lo / 2) + (hi / 2);
	    int comp = _data[med].compareTo(newVal);
	    if (comp < 0) {
		return findBinH(newVal, lo, med - 1);}
	    if (comp > 0) {
		return findBinH(newVal,  med + 1, hi);}
	    return med;
	}
